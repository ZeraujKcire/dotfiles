...
ks.test(residuales,"pnorm",0, sqrt(anova(regresion)[3,3]))
    ...
    # X1         1   68.6    68.6  0.6776    0.4225    
    # X2         1 4111.4  4111.4 40.6342 9.214e-06 ***
    # X3         1   87.8    87.8  0.8676    0.3655    
    # Residuals 16 1618.9   [101.2]                     <--
    # ===============================================================
# )))
...
# === 12 PRUEBAS DE HIPÓTESIS A COEICIENTES === (((
sprintf(" --- 12 PRUEBAS DE HIPOTESIS A COEFICIENTES --- ")
n = dim(anova)[1] # véase que se tienen 4 filas en la tabla anova (línea 64).
qt(c(0.025,0.975),df=anova[n,1]) # se obtiene el percentil de la dsitribución T.
# )))

# Se reduce el modelo para X2(VOL)
regresion=lm(Y~X1,na.action="na.exclude")
