...
function createLinkItem (link, rect, key) {
  ...
  item.setAttribute('style', 'position: absolute; padding: 1px 3px 0px 3px; background-color: yellow; color: black; z-index: 9999; font-family: Victor Mono Semibold;font-weight: bold;font-size: 12px; background: linear-gradient(to bottom, #FFF785 0%,#FFC542 100%); border: solid 1px #C38A22; border-radius: 3px; box-shadow: 0px 3px 7px 0px rgba(0, 0, 0, 0.3);')
...
document.addEventListener('keydown', function (e) {
  if ((e.key === 'k' || e.key === 'l') && !isLinkKeyMode && !isCurrentlyInInput()) {
    if (e.key === 'k') {
...
const commandChars = new Set(['f', 'F', 'y', 'g', 'K', 'L', 'c'])
